...
def get_event_from_csv(pdtx_df, window_timestamp_start, window_timestamp_end):
    ...
    pdtx_df_part = pdtx_df[(pdtx_df['curblocknumber']>=window_timestamp_start) & (pdtx_df['curblocknumber']<=window_timestamp_end)].reset_index(drop=True)
    # 时间段内所有代币转移事件
    window_events={}
        ...
        for log in logs_json:
            if len(log['topics'])>0:
                if (log['topics'][0]==Transfer) or (log['topics'][0]==ERC20Transfer):
                    ...
                    # cur_log["log_timestamp"]=getTimestampByStr(pdtx['detecttime'])
...
def getTimestampByStr(timeStr):
    ...
    # dt = datetime.strptime(timeStr, "%Y-%m-%d %H:%M:%S.%f%z")
...
def detectSandwichAttackByHeuristic(time_str, pdtx_df):
    ...
    start_blockNumber = int(pdtx_df['curblocknumber'].values[0])
    end_blockNumber = int(pdtx_df['curblocknumber'].values[-1])
    ...
    for bn in tqdm(range(start_blockNumber, end_blockNumber), desc=str(time_str)):
        cur_res_list = []
...
'''
def detectSandwich(time_str, mempool_time2blocknumber):
    def has_overlap(interval1, interval2):
        start1, end1 = interval1
        start2, end2 = interval2
        return not (end1 < start2 or end2 < start1)
    mem_file_list = []
    for key ,value in mempool_time2blocknumber.items():
        if has_overlap([time_str,time_str+10000], value):
            mem_file_list.append(key)
    pdtx_df = pd.DataFrame([])
    for mfl in mem_file_list:
        mfl_df = pd.read_csv(f'SandwichAttackInMempoolTest/{mfl}.csv')
        # if 'curblocknumber' in mfl_df.columns:
        pdtx_df = pd.concat([pdtx_df, mfl_df])
    pdtx_df = pdtx_df[(pdtx_df['curblocknumber']>=time_str) & (pdtx_df['curblocknumber']<time_str+10000)]
    
    pdtx_df = pdtx_df.sort_values(by='curblocknumber').reset_index(drop=True)
    start_blockNumber = int(pdtx_df['curblocknumber'].values[0])
    end_blockNumber = int(pdtx_df['curblocknumber'].values[-1])
...
if __name__ == '__main__':
    ...
    full_path = f'Output/Mempool_{current_time_str}'
    full_path = 'Output/Mempool_2024-10-21 19:40:07'
    ...
    mempool_files = sorted(glob('SandwichAttackInMempoolTest/*.csv'))
    with open('labeled_dataset/mempool_time2blocknumber.json', 'r') as f:
        mempool_time2blocknumber = json.load(f)
        
    # for time_str in range(19000000, 19800000, 10000):
    # for time_str in range(19600000, 19910000, 10000):
    # for time_str in range(19770000, 19910000, 10000):
    for time_str in range(19500000, 19600000, 10000):
        detectSandwich(time_str, mempool_time2blocknumber)
